          ...
          <div className="bg-neutral-900 border border-orange-500/40 rounded-3xl p-6 md:p-8 max-w-2xl w-full relative shadow-2xl space-y-6 max-h-[90vh] overflow-y-auto">
            ...
            {!profile ? (
              ...
              <div className="space-y-6">
                  ...
                  {isEditing && (
                    <div className="space-y-4 bg-black/40 p-5 rounded-2xl border border-orange-500/20">
                      ...
                      <div className="flex justify-end space-x-2 pt-2">
                        ...
                        <button onClick={handleSaveProfile} className="px-4 py-2 bg-orange-500 hover:bg-orange-400 text-black font-black text-xs uppercase rounded-lg">Сохранить</button>
                      </div>
                    </div>
                  )}
                </div>
                ...
                {/* БАЛАНС ГОЛДЫ */}
                <div className="bg-gradient-to-r from-amber-950/40 via-amber-900/20 to-black border border-amber-500/40 p-5 rounded-2xl flex flex-col sm:flex-row items-center justify-between gap-4 shadow-lg">
                  <div className="flex items-center space-x-3">
                    ...
                    <div>
                      <span className="text-xs text-gray-400 font-bold uppercase tracking-wider block">Баланс золота</span>
                      <span className="text-2xl font-black text-amber-400">{profile.goldBalance || 0} GOLD</span>
                    </div>
                  </div>

                  <button 
                    onClick={() => setActiveModal('withdraw')}
                    className="w-full sm:w-auto px-5 py-3 bg-gradient-to-r from-amber-500 to-yellow-500 hover:brightness-110 text-black font-black text-xs uppercase rounded-xl transition-all shadow-lg flex items-center justify-center space-x-2"
                  >
                    <ArrowDownRight className="w-4 h-4" />
                    <span>Вывести золото</span>
                  </button>
                </div>

                {/* ИГРОВАЯ СТАТИСТИКА */}
                <div className="bg-black/60 border border-orange-500/30 p-5 rounded-2xl space-y-4">
                  <div className="flex items-center justify-between">
                    <h4 className="text-xs font-black uppercase text-amber-400 tracking-wider">Игровая статистика</h4>
                  </div>

                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 text-center">
                    <div className="bg-white/5 border border-white/10 p-3 rounded-xl">
                      <span className="text-[10px] text-gray-400 font-bold uppercase block">Победные матчи</span>
                      <span className="text-lg font-black text-emerald-400">{profile.wins}</span>
                    </div>
                    <div className="bg-white/5 border border-white/10 p-3 rounded-xl">
                      <span className="text-[10px] text-gray-400 font-bold uppercase block">К/Д (K/D)</span>
                      <span className="text-lg font-black text-amber-400">
                        {profile.deaths > 0 ? (profile.kills / profile.deaths).toFixed(2) : profile.kills.toFixed(2)}
                      </span>
                    </div>
                    <div className="bg-white/5 border border-white/10 p-3 rounded-xl">
                      <span className="text-[10px] text-gray-400 font-bold uppercase block">Убийства (Килы)</span>
                      <span className="text-lg font-black text-white">{profile.kills}</span>
                    </div>
                    <div className="bg-white/5 border border-white/10 p-3 rounded-xl">
                      <span className="text-[10px] text-gray-400 font-bold uppercase block">Выведено голды</span>
                      <span className="text-lg font-black text-yellow-400">{profile.totalWithdrawn || 0} G</span>
                    </div>
                  </div>
                </div>

                {/* ИСТОРИЯ МАТЧЕЙ И ИСТОРИЯ ВЫИГРЫШЕЙ ПОД КНОПКОЙ */}
                <div className="space-y-4 pt-2">
                  {/* Кнопка и блок: История матчей */}
                  <div>
                    <button 
                      onClick={() => setIsMatchHistoryOpen(!isMatchHistoryOpen)}
                      className="w-full py-3.5 px-4 bg-neutral-800 hover:bg-neutral-700 border border-white/10 rounded-2xl flex items-center justify-between text-xs font-black uppercase tracking-wider text-white transition-all shadow-md"
                    >
                      <div className="flex items-center space-x-2">
                        <Swords className="w-4 h-4 text-orange-400" />
                        <span>История матчей</span>
                      </div>
                      {isMatchHistoryOpen ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
                    </button>

                    {isMatchHistoryOpen && (
                      <div className="mt-2 bg-black/60 border border-white/10 rounded-2xl p-4 space-y-3 animate-fadeIn">
                        <div className="flex justify-between items-center bg-white/5 p-3 rounded-xl text-xs">
                          <div>
                            <span className="font-bold text-white block">Standoff 2 Cup - 1/8 Финала</span>
                            <span className="text-[10px] text-gray-400">Sandstone • Победа</span>
                          </div>
                          <span className="text-emerald-400 font-black">+50 G / 10-6</span>
                        </div>
                        <div className="flex justify-between items-center bg-white/5 p-3 rounded-xl text-xs">
                          <div>
                            <span className="font-bold text-white block">Fast Cup 2x2</span>
                            <span className="text-[10px] text-gray-400">Sakura • Победа</span>
                          </div>
                          <span className="text-emerald-400 font-black">+100 G / 10-4</span>
                        </div>
                      </div>
                    )}
                  </div>

                  {/* Кнопка и блок: История выигрышей */}
                  <div>
                    <button 
                    ...
                    >
                      <div className="flex items-center space-x-2">
                        ...
                        <span>История выигрышей</span>
                      </div>
                      {isWinHistoryOpen ? <ChevronUp className="w-4 h-4" /> : <ChevronDown className="w-4 h-4" />}
                    </button>

                    {isWinHistoryOpen && (
                      <div className="mt-2 bg-black/60 border border-white/10 rounded-2xl p-4 space-y-3 animate-fadeIn">
                        {winHistory.length === 0 ? (
                          <p className="text-xs text-gray-400 text-center py-2">История выигрышей пуста</p>
                        ) : (
                          winHistory.map((item) => (
                            <div key={item.id} className="flex justify-between items-center bg-white/5 p-3 rounded-xl text-xs">
                              ...
                              <span className="text-amber-400 font-black">{item.reward}</span>
                            </div>
                          ))
                        )}
                      </div>
